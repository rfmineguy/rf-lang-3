...
int lalr_reduce(lalr_ctx* ctx, AST_Node* out_n) {
	...
	int popCount = 0;
	if ((popCount = lalr_try_reduce_number(ctx, out_n))        		!= 0) return popCount;
	if ((popCount = lalr_try_reduce_header(ctx, out_n))        		!= 0) return popCount;
	if ((popCount = lalr_try_reduce_vartype(ctx, out_n))       		!= 0) return popCount;
	if ((popCount = lalr_try_reduce_typed_id(ctx, out_n))      		!= 0) return popCount;
	if ((popCount = lalr_try_reduce_typed_id_list(ctx, out_n)) 		!= 0) return popCount;
	if ((popCount = lalr_try_reduce_deref(ctx, out_n))     				!= 0) return popCount;
	if ((popCount = lalr_try_reduce_addressof(ctx, out_n)) 				!= 0) return popCount;
	if ((popCount = lalr_try_reduce_factor(ctx, out_n))    				!= 0) return popCount;
	if ((popCount = lalr_try_reduce_term(ctx, out_n)) 		 				!= 0) return popCount;
	if ((popCount = lalr_try_reduce_math_expr(ctx, out_n)) 				!= 0) return popCount;
	if ((popCount = lalr_try_reduce_relate(ctx, out_n)) 					!= 0) return popCount;
	if ((popCount = lalr_try_reduce_logical_conj(ctx, out_n)) 		!= 0) return popCount;
	if ((popCount = lalr_try_reduce_logical_disj(ctx, out_n)) 		!= 0) return popCount;
	if ((popCount = lalr_try_reduce_expr(ctx, out_n)) 						!= 0) return popCount;
	if ((popCount = lalr_try_reduce_return(ctx, out_n)) 					!= 0) return popCount;
	if ((popCount = lalr_try_reduce_expr_list(ctx, out_n)) 				!= 0) return popCount;
	if ((popCount = lalr_try_reduce_array_index(ctx, out_n)) 			!= 0) return popCount;
	if ((popCount = lalr_try_reduce_func_call(ctx, out_n)) 				!= 0) return popCount;
	if ((popCount = lalr_try_reduce_function_header(ctx, out_n)) 	!= 0) return popCount;
	if ((popCount = lalr_try_reduce_if(ctx, out_n)) 							!= 0) return popCount;
	if ((popCount = lalr_try_reduce_assignment(ctx, out_n)) 			!= 0) return popCount;
	if ((popCount = lalr_try_reduce_statement_list(ctx, out_n)) 	!= 0) return popCount;
	if ((popCount = lalr_try_reduce_statement(ctx, out_n)) 				!= 0) return popCount;
	if ((popCount = lalr_try_reduce_block(ctx, out_n)) 						!= 0) return popCount;
	if ((popCount = lalr_try_reduce_function(ctx, out_n)) 				!= 0) return popCount;
	return 0;
}
...
/** number
	...
	* 									| <hexlit>
 */
int lalr_try_reduce_number(lalr_ctx* ctx, AST_Node* out_n) {
	AST_Node peeked[5] = {lalr_peek_n(ctx, 0), lalr_peek_n(ctx, 1), lalr_peek_n(ctx, 2), lalr_peek_n(ctx, 3),  lalr_peek_n(ctx, 4)};
	token_type lookahead = ctx->lookahead.type;
 ...
 */
int lalr_try_reduce_header(lalr_ctx* ctx, AST_Node* out_n) {
	AST_Node peeked[5] = {lalr_peek_n(ctx, 0), lalr_peek_n(ctx, 1), lalr_peek_n(ctx, 2), lalr_peek_n(ctx, 3),  lalr_peek_n(ctx, 4)};
	token_type lookahead = ctx->lookahead.type;
 ...
 *    vartype := "*" <vartype>
 */
int lalr_try_reduce_vartype(lalr_ctx* ctx, AST_Node* out_n) {
	AST_Node peeked[5] = {lalr_peek_n(ctx, 0), lalr_peek_n(ctx, 1), lalr_peek_n(ctx, 2), lalr_peek_n(ctx, 3),  lalr_peek_n(ctx, 4)};
	token_type lookahead = ctx->lookahead.type;
	// vartype := <id>       NOTE: with context of previous and next tokens
	if (peeked[0].type == NT_TOKEN && peeked[0].token.type == T_ID &&
			...
			lookahead != T_LP && lookahead != T_PLUS && lookahead != T_MINUS && lookahead != T_MUL && lookahead != T_DIV && lookahead != T_MOD) {
 ...
 * 		typed_id := <id> ":" <vartype>
 */
int lalr_try_reduce_typed_id(lalr_ctx* ctx, AST_Node* out_n) {
	AST_Node peeked[5] = {lalr_peek_n(ctx, 0), lalr_peek_n(ctx, 1), lalr_peek_n(ctx, 2), lalr_peek_n(ctx, 3),  lalr_peek_n(ctx, 4)};
	token_type lookahead = ctx->lookahead.type;
 ...
 */
int lalr_try_reduce_typed_id_list(lalr_ctx* ctx, AST_Node* out_n) {
	AST_Node peeked[5] = {lalr_peek_n(ctx, 0), lalr_peek_n(ctx, 1), lalr_peek_n(ctx, 2), lalr_peek_n(ctx, 3),  lalr_peek_n(ctx, 4)};
	token_type lookahead = ctx->lookahead.type;
	//   typed_id_list := <typed_id_list> "," <typed_id>
 ...
 *    deref := <factor>
 */
int lalr_try_reduce_deref(lalr_ctx* ctx, AST_Node* out_n) {
	AST_Node peeked[5] = {lalr_peek_n(ctx, 0), lalr_peek_n(ctx, 1), lalr_peek_n(ctx, 2), lalr_peek_n(ctx, 3),  lalr_peek_n(ctx, 4)};
	token_type lookahead = ctx->lookahead.type;
	...
	if (peeked[0].type == NT_FACTOR) {
		...
		out_n->deref->type = DEREF_TYPE_FACTOR;
		out_n->deref->f = peeked[0].factor;
		out_n->deref->loc = peeked[0].factor.loc;
		return 1;
	}
	return 0;
}
 ...
 *    addressof := <deref>
 */
int lalr_try_reduce_addressof(lalr_ctx* ctx, AST_Node* out_n) {
	AST_Node peeked[5] = {lalr_peek_n(ctx, 0), lalr_peek_n(ctx, 1), lalr_peek_n(ctx, 2), lalr_peek_n(ctx, 3),  lalr_peek_n(ctx, 4)};
	token_type lookahead = ctx->lookahead.type;
 ...
 *     factor := <array_index>
 */
int lalr_try_reduce_factor(lalr_ctx* ctx, AST_Node* out_n) {
	AST_Node peeked[5] = {lalr_peek_n(ctx, 0), lalr_peek_n(ctx, 1), lalr_peek_n(ctx, 2), lalr_peek_n(ctx, 3),  lalr_peek_n(ctx, 4)};
	token_type lookahead = ctx->lookahead.type;
	...
	// factor := <array_index>
	if (peeked[0].type == NT_ARRAY_INDEX) {
		out_n->type = NT_FACTOR;
 ...
 *	   term := <addressof>
 */
int lalr_try_reduce_term(lalr_ctx* ctx, AST_Node* out_n) {
	AST_Node peeked[5] = {lalr_peek_n(ctx, 0), lalr_peek_n(ctx, 1), lalr_peek_n(ctx, 2), lalr_peek_n(ctx, 3),  lalr_peek_n(ctx, 4)};
	token_type lookahead = ctx->lookahead.type;
 ...
 */
int lalr_try_reduce_math_expr(lalr_ctx* ctx, AST_Node* out_n) {
	AST_Node peeked[5] = {lalr_peek_n(ctx, 0), lalr_peek_n(ctx, 1), lalr_peek_n(ctx, 2), lalr_peek_n(ctx, 3),  lalr_peek_n(ctx, 4)};
	token_type lookahead = ctx->lookahead.type;
	// math_expression := <math_expression> "+/-" <term>
 ...
 */
int lalr_try_reduce_relate(lalr_ctx* ctx, AST_Node* out_n) {
	AST_Node peeked[5] = {lalr_peek_n(ctx, 0), lalr_peek_n(ctx, 1), lalr_peek_n(ctx, 2), lalr_peek_n(ctx, 3),  lalr_peek_n(ctx, 4)};
	token_type lookahead = ctx->lookahead.type;
	// relational := relational ">=" math_expression
	// relational := relational "<=" math_expression
 ...
 */
int lalr_try_reduce_logical_conj(lalr_ctx* ctx, AST_Node* out_n) {
	AST_Node peeked[5] = {lalr_peek_n(ctx, 0), lalr_peek_n(ctx, 1), lalr_peek_n(ctx, 2), lalr_peek_n(ctx, 3),  lalr_peek_n(ctx, 4)};
	token_type lookahead = ctx->lookahead.type;
 ...
 */
int lalr_try_reduce_logical_disj(lalr_ctx* ctx, AST_Node* out_n) {
	AST_Node peeked[5] = {lalr_peek_n(ctx, 0), lalr_peek_n(ctx, 1), lalr_peek_n(ctx, 2), lalr_peek_n(ctx, 3),  lalr_peek_n(ctx, 4)};
	token_type lookahead = ctx->lookahead.type;
 ...
 */
int lalr_try_reduce_expr(lalr_ctx* ctx, AST_Node* out_n) {
	AST_Node peeked[5] = {lalr_peek_n(ctx, 0), lalr_peek_n(ctx, 1), lalr_peek_n(ctx, 2), lalr_peek_n(ctx, 3),  lalr_peek_n(ctx, 4)};
	token_type lookahead = ctx->lookahead.type;
 ...
 *    return <expression>
 */
int lalr_try_reduce_return(lalr_ctx* ctx, AST_Node* out_n) {
	AST_Node peeked[5] = {lalr_peek_n(ctx, 0), lalr_peek_n(ctx, 1), lalr_peek_n(ctx, 2), lalr_peek_n(ctx, 3),  lalr_peek_n(ctx, 4)};
	token_type lookahead = ctx->lookahead.type;
 ...
 */
int lalr_try_reduce_expr_list(lalr_ctx* ctx, AST_Node* out_n) {
	AST_Node peeked[5] = {lalr_peek_n(ctx, 0), lalr_peek_n(ctx, 1), lalr_peek_n(ctx, 2), lalr_peek_n(ctx, 3),  lalr_peek_n(ctx, 4)};
	token_type lookahead = ctx->lookahead.type;
 ...
 */
int lalr_try_reduce_array_index(lalr_ctx* ctx, AST_Node* out_n) {
	AST_Node peeked[5] = {lalr_peek_n(ctx, 0), lalr_peek_n(ctx, 1), lalr_peek_n(ctx, 2), lalr_peek_n(ctx, 3),  lalr_peek_n(ctx, 4)};
	token_type lookahead = ctx->lookahead.type;
	...
	if (peeked[3].type == NT_TOKEN &&
		...
		out_n->type = NT_ARRAY_INDEX;
		out_n->arrayIndex = arena_alloc(&ctx->arena, sizeof(Deref));
		// out_n->arrayIndex->type = DEREF_TYPE_BRKT;
		out_n->arrayIndex->Brkt.exprList = peeked[1].exprList;
		out_n->arrayIndex->Brkt.id = peeked[3].token.text;
		out_n->arrayIndex->loc = peeked[3].token.loc;
		return 4;
	}
	...
	if (peeked[3].type == NT_TOKEN &&
		...
		// out_n->arrayIndex->type = DEREF_TYPE_BRKT;
		out_n->arrayIndex->Brkt.id = peeked[3].token.text;
		out_n->arrayIndex->Brkt.exprList = arena_alloc(&ctx->arena, sizeof(ExpressionList));
		out_n->arrayIndex->Brkt.exprList->type = EXPRESSION_LIST_TYPE_EXPR;
		out_n->arrayIndex->Brkt.exprList->expr = peeked[1].expr;
		out_n->arrayIndex->Brkt.exprList->next = NULL;
		out_n->arrayIndex->loc = peeked[3].token.loc;
		return 4;
	}
 ...
 */
int lalr_try_reduce_func_call(lalr_ctx* ctx, AST_Node* out_n) {
	AST_Node peeked[5] = {lalr_peek_n(ctx, 0), lalr_peek_n(ctx, 1), lalr_peek_n(ctx, 2), lalr_peek_n(ctx, 3),  lalr_peek_n(ctx, 4)};
	token_type lookahead = ctx->lookahead.type;
 ...
 */
int lalr_try_reduce_function_header(lalr_ctx* ctx, AST_Node* out_n) {
	AST_Node peeked[5] = {lalr_peek_n(ctx, 0), lalr_peek_n(ctx, 1), lalr_peek_n(ctx, 2), lalr_peek_n(ctx, 3),  lalr_peek_n(ctx, 4)};
	token_type lookahead = ctx->lookahead.type;
 ...
 */
int lalr_try_reduce_if(lalr_ctx* ctx, AST_Node* out_n) {
	AST_Node peeked[5] = {lalr_peek_n(ctx, 0), lalr_peek_n(ctx, 1), lalr_peek_n(ctx, 2), lalr_peek_n(ctx, 3),  lalr_peek_n(ctx, 4)};
	token_type lookahead = ctx->lookahead.type;
 ...
 */
int lalr_try_reduce_assignment(lalr_ctx* ctx, AST_Node* out_n) {
	AST_Node peeked[5] = {lalr_peek_n(ctx, 0), lalr_peek_n(ctx, 1), lalr_peek_n(ctx, 2), lalr_peek_n(ctx, 3),  lalr_peek_n(ctx, 4)};
	token_type lookahead = ctx->lookahead.type;
 ...
 */
int lalr_try_reduce_statement_list(lalr_ctx* ctx, AST_Node* out_n) {
	AST_Node peeked[5] = {lalr_peek_n(ctx, 0), lalr_peek_n(ctx, 1), lalr_peek_n(ctx, 2), lalr_peek_n(ctx, 3),  lalr_peek_n(ctx, 4)};
	token_type lookahead = ctx->lookahead.type;
 ...
 */
int lalr_try_reduce_statement(lalr_ctx* ctx, AST_Node* out_n) {
	AST_Node peeked[5] = {lalr_peek_n(ctx, 0), lalr_peek_n(ctx, 1), lalr_peek_n(ctx, 2), lalr_peek_n(ctx, 3),  lalr_peek_n(ctx, 4)};
	token_type lookahead = ctx->lookahead.type;
 ...
 */
int lalr_try_reduce_block(lalr_ctx* ctx, AST_Node* out_n) {
	AST_Node peeked[5] = {lalr_peek_n(ctx, 0), lalr_peek_n(ctx, 1), lalr_peek_n(ctx, 2), lalr_peek_n(ctx, 3),  lalr_peek_n(ctx, 4)};
	token_type lookahead = ctx->lookahead.type;
 ...
 */
int lalr_try_reduce_function(lalr_ctx* ctx, AST_Node* out_n) {
	AST_Node peeked[5] = {lalr_peek_n(ctx, 0), lalr_peek_n(ctx, 1), lalr_peek_n(ctx, 2), lalr_peek_n(ctx, 3),  lalr_peek_n(ctx, 4)};
	token_type lookahead = ctx->lookahead.type;
